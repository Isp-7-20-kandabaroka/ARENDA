...
async def add_city(message: types.Message, state: FSMContext):
    ...
    # Сохранение названия города и ID пользователя в состояние не требуется
...
async def done_add(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect('my_database.db') as db:
        ...
        try:
            ...
            expiration_text = "2 месяца" if ads_count < 500 else "14 дней"
            message_text = f"Ваше объявление с ID {ad_id} размещено на срок {expiration_text}.\n\nОписание: {description}\nКонтакт: {contact}"
